...
| HO-DET-011 | Windows service creation or service binary change | Sigma + Wazuh XML + Splunk SPL | `.yml` + `.xml` + `.spl` | Windows Security Event ID 7045 and Sysmon Event ID 1 | `VALIDATION_PLANNED` | none | Sigma source plus Wazuh XML rule plus SPL covering Event ID 7045 and image-path heuristics | Synthetic fixture set distinguishing benign service install from suspicious binary path | Runtime evidence not in scope at this stage | runtime-active, signal-observed, evidence-linked public proof, public-safe |
